...
describe("applyBusinessFilters", () => {
  it(
    "returns empty result metadata when price filter removes all candidates",
    () => {
  ...
  it("applies exclusive and inclusive price constraints correctly", () => {
    ...
    expect(
      inclusiveResult.filteredMatches.map((item) => item.id),
    ).toEqual(["a", "b"]);
    expect(
      exclusiveResult.filteredMatches.map((item) => item.id),
    ).toEqual(["b"]);
  });

  it(
    "uses first candidate fallback when distance cutoff excludes all matches",
    () => {
      const rawMatches = [
